//Non-Volatile Memory Library Function Definitions Version 5
...
//Version 5 Improvements from Version 3:
...
//1. Uses binary files for int* data instead of text files for char data
//2. pwritef takes an int not a string
...
typedef struct oid
{
	int* data; //place to store int data
...
void pwritef(pool* p, int num)
{
	...
	{
		tmp->data = (int*) num; //writes num to data
		tmp->empty = 0;
	}
}
...
void preadf(pool* p)
{
	OID * tmp = p->root;
	int i = 0;
	while (tmp->empty != 1) //cycles through all non-empty oids in pool
	{
		printf("offset: %d\n", i);
		printf("%d\n", (int) tmp->data); //prints data and offset values of oid
		i++;
		if (i < p->size)
		{
			tmp = tmp->next;
...
void pfilein(pool* p, char* filename)
{
	...
	{
		FILE* file_ptr = fopen(filename, "rb"); //opens binary file to be read
		int i = tmp->offset;	
		while (fread(&(tmp->data), sizeof(int), 1, file_ptr) == 1) //reads all binary file data to oid
		{
			tmp->empty = 0;
			if (tmp->next == NULL)
			{
...
{
	FILE* file_ptr = fopen(filename, "wb"); //opens binary file to be written to
	OID * tmp = p->root;
	while (tmp->empty != 1) //cycles through all non-empty oids of pool
	{
		fwrite(&(tmp->data), sizeof(int), 1, file_ptr); //writes data to binary file
		if (tmp->next == NULL)
		{
			break;
		}
